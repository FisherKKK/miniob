...
RC PaxRecordPageHandler::insert_record(const char *data, RID *rid)
{
  ...
  int32_t offset = 0;
  for (int ci = 0; ci < page_header_->column_num; ci++) {
    char* record_ci = get_field_data(index, ci);
    int32_t field_len = get_field_len(ci);
    memcpy(record_ci, data + offset, field_len);
    offset += field_len;
  }
  frame_->mark_dirty();
...
RC PaxRecordPageHandler::get_record(const RID &rid, Record &record)
{
  ...
  char data[page_header_->record_real_size];
  int32_t offset = 0;
  for (int ci = 0; ci < page_header_->column_num; ci++) {
    int32_t field_len = get_field_len(ci);
    memcpy(data + offset, get_field_data(rid.slot_num, ci), field_len);
    offset += field_len;
  }
  record.copy_data(data, offset);
  record.set_rid(rid);
  return RC::SUCCESS;
}
...
{
  Bitmap bitmap(bitmap_, page_header_->record_capacity);
  for (int ci = 0; ci < chunk.column_num(); ci++) {
    auto &col= chunk.column(ci);
    for (int ri = 0; ri < page_header_->record_capacity; ri++) {
      if (bitmap.get_bit(ri)) {
        char *data = get_field_data(ri, chunk.column_ids(ci));
        col.append_one(data);
      }
    }
  }
